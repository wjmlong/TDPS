import numpy as np
import cv2 as cv
import matplotlib.pyplot as plt
import math as m
import time

def edge_demo(image,t1,c,td,h,s):
    
    red = (0, 0, 255)
    blue = (255,0,0)

    #gray = cv.cvtColor(blurred, cv.COLOR_RGB2GRAY)

    Lower1 = np.array([150,80,30])
    Upper1 = np.array([170,255,255])
    Lower2 = np.array([25,80,50])
    Upper2 = np.array([77,255,255])
    Lower3 = np.array([100,100,50])
    Upper3 = np.array([130,255,255])
    edge_outputr = cv.inRange(image,Lower1,Upper1)
    edge_outputg = cv.inRange(image,Lower2,Upper2)
    edge_outputb = cv.inRange(image,Lower3,Upper3)
    cv.imshow("r", edge_outputr)
    output1 = cv.cvtColor(edge_outputr, cv.COLOR_GRAY2RGB)
    cv.imshow("g", edge_outputg)
    cv.imshow("b", edge_outputb)
    #edge_output2 = cv.Canny(blurred, 50, 50)#gray法
    
    kernel = cv.getStructuringElement(cv.MORPH_RECT, (5, 5), (-1, -1))
    kernel2 = cv.getStructuringElement(cv.MORPH_RECT, (19, 19), (-1, -1))

    eroder = cv.erode(edge_outputr, kernel)
    cv.imshow("eroder", eroder)
    dilater = cv.dilate(eroder, kernel2)
    cv.imshow("dilater", dilater)
    
    M = cv.moments(dilater)
    if(M["m00"] == 0 or M["m00"] == 0):
     print('no')
     cv.imshow("r1", output1)
    else:
     cX = int(M["m10"] / M["m00"]);
     cY = int(M["m01"] / M["m00"]);
     print(cX,cY)
     
     
     cv.circle(output1,(cX,cY),25,(255,0,255),1)
     cv.imshow("r1", output1)
    

    erodeg = cv.erode(edge_outputg, kernel)
    cv.imshow("erodeg", erodeg)
    dilateg = cv.dilate(erodeg, kernel2)
    cv.imshow("dilateg", dilateg)
    
    erodeb = cv.erode(edge_outputb, kernel)
    cv.imshow("erodeb", erodeb)
    dilateb = cv.dilate(erodeb, kernel2)
    cv.imshow("dilateb", dilateb)


    edge_output3 = cv.cvtColor(edge_outputr, cv.COLOR_GRAY2RGB)

    
    font = cv.FONT_HERSHEY_DUPLEX
    c2 = int(c)
    cv.line(edge_output3,(c2,0),(c2,480),red,3)
    cv.line(edge_output3,(220,0),(220,480),blue,1)
    cv.line(edge_output3,(420,0),(420,480),blue,1)

    c1 = 'direction = ''%.2f' %c
    cv.putText(edge_output3, c1,(0,25),font,1,(0,255,255),1,8)
    t = 'lower limit = ''%d'%t1
    cv.putText(edge_output3, t,(0,55),font,1,(0,255,255),1,8)
    td = 'delay = ''%.2f' %td
    cv.putText(edge_output3, td,(0,85),font,1,(0,255,255),1,8)
    cv.imshow("Canny Edge3", edge_output3)

    return edge_outputr

"""
def sam(n,h,w,img):
    a = [0 for i in range(1,n)]96
        for i in range(s1, s2):
              for j in range(int(m.floor(h/2)),h):
                    if img[j,i] > 100:
                         a[x - 1] = a[x - 1] + 1

    return [a, s]
 """
def sam(n,h,w,img):

    a = [0 for i in range(1,n)]
    s = [0 for i in range(1,n)]
    for x in range(1,n):
        s1 = int(m.floor(w/n*x))
        s2 = s1 + int(m.floor(w/5/n))
        s[x - 1] = (s1 + s2)/2
        a[x - 1] = int(np.sum(img[int(h/2):h,s1:s2]))
    return [a, s]
   
def draw(a,s,n):
    blue = (255,255,0)
    av = int(np.mean(a))
    a = np.subtract(a, av)
    graph = np.zeros((480, 640, 3), dtype="uint8")
    for i in range(0,n - 1):
           if(a[i]<0):
                 a[i] = 0
           x =  int(640/(n + 1)*(i + 1))
           cv.line(graph,(x,480),(x,480 - int(a[i]/20)),blue,3)

    s1 = sum(a)
    if s1 == 0:
        c = 320
    else: 
        c = np.multiply(a,s)
        c = sum(c/s1)
    ci = int(c)

    cv.line(graph,(ci,0),(ci,480),(0,0,255),3)
    cv.imshow("graph",graph)
    #plt.plot(s,a,'-g')    
    #plt.plot([c,c],[0,max(a)],'red') 
    #plt.pause(0.1)
    #plt.cla()
    return s1,c

def adj(s):
    if(s >= 290)and(s <= 320):
        t1 = 50
    elif(s >= 250)and(s < 290):
        t1 = 30
    elif(s > 320)and(s <= 400):
        t1 = 70
    elif(s < 250):
        t1 = 20
    elif(s > 400):
        t1 = 100
    return t1    

def phsv(img):
    HSV = cv.cvtColor(img, cv.COLOR_BGR2HSV)
    imgh = HSV[:,:,0]
    imgs = HSV[:,:,1]
    imgv = HSV[:,:,2]
    
    s, dsts = cv.threshold(imgs, 0, 255, cv.THRESH_OTSU)
    h, dsth = cv.threshold(imgh, 0, 255, cv.THRESH_OTSU)
    print(h,s)
    #cv.imshow('dst',dst)
    return int(h),int(s),HSV
    

cap = cv.VideoCapture(0)
cap.set(3,640) # set Width
cap.set(4,480) # set Height
t1 = 50
c = 320
td = 0
while(True):
    
    time_start = time.time()
    n = 50
    ret,frame = cap.read()
    cv.imshow('frame', frame)
    h,s,HSV = phsv(frame)
    img = edge_demo(HSV,t1,c,td,h,s)
    [h,w] = img.shape
    [a,s] = sam(n,h,w,img)
    s1,c = draw(a,s,n) 
    t1 = adj(s1)
    print('\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n')
    if(c > 420):        
        if(c > 500):
            print(c,'left2')
        else:
            print(c,'left')
    elif(c < 220):
        if(c <150):
             print(c,'right2')
        else:
             print(c,'right')
    else:
        print(c,'middle')

    print('lower limit = ',t1,'the total point = ',s1,'direction = ',c)
    time_end = time.time()
    td = time_end-time_start
    time_start = 0
    k = cv.waitKey(30) & 0xff
    if k == 27:
        break
cap.release()
cv.destroyAllWindows()
